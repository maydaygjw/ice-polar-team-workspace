...
正式功能进入本阶段时使用 `$first-ask` 进行交互澄清：

1. 先阅读用户输入、现有治理文档、相关功能规格和必要的代码事实，避免询问可直接查明的信息。
2. 围绕范围与目标、交付物、业务规则、异常场景、约束和验收标准逐轮澄清；问题一次最多 3 个，标明是否阻塞、影响和建议默认处理。
3. 用户回答后同步已确认的结论，只继续追问仍会改变业务行为或交付范围的歧义。
4. 澄清完成后、产出规格前，询问用户是否还有补充；持续澄清直至用户明确表示没有补充。
5. 澄清结束后向用户展示最精简的执行计划，创建 Todo，再开始编写 `requirements-spec.md`。

`$first-ask` 依赖的 Joyride 输入工具不可用时，使用普通对话提问完成同等流程，不因工具缺失阻塞需求阶段。

门禁：范围明确；验收标准可验证；高风险歧义已确认。需求完整时直接记录规格和假设，不为满足流程强制提问；仍需在开始产出前完成“是否还有补充”的最终确认。
