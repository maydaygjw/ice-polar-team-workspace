...
3. 模板通过 `ProductImportTemplateParser` 适配器注册；模板元数据保存 `parserCode`，导入时以模板编码查元数据，再委托对应解析器。`meituan-user`（版本 v1）优先读取多规格商品详情 Sheet。
...
9. 首期图片只保存源 URL，避免在导入事务中引入外部图片下载失败和超时。
