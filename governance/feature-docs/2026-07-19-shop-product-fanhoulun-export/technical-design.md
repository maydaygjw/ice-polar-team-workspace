...
| 7 | 商品库存 | `stock` | `StoreProductDO.stock`，`-1` 输出 `100000` | 空 |
| 8 | 餐盒费 | `boxFee` | `StoreProductDO.boxFee` | 同单规格 |
| 9 | 是否招牌 | `isHot` | 固定 `2` | 固定 `2` |
| 10 | 是否上架 | `isShow` | `StoreProductDO.isShow` | 同单规格 |
| 11 | 原价 | `originalPrice` | `StoreProductDO.otPrice` | `StoreProductDO.otPrice` |
| 12 | 商品销量 | `sales` | `StoreProductDO.sales` | `StoreProductDO.sales` |
| 13-32 | 规格维度/值 1-10 | `specDim1`/`specValue1` … `specDim10`/`specValue10` | 空 | 属性名称与 SKU 值按顺序填充 |
| 33 | SKU外卖价格 | `skuPrice` | 空 | `StoreProductAttrValueDO.price` |
| 34 | SKU店内价格 | `skuOtPrice` | 空 | 与 `skuPrice` 相同 |
| 35 | SKU库存 | `skuStock` | 空 | `StoreProductAttrValueDO.stock`，`-1` 输出 `100000` |

## 类设计

### VO

- `FHLProductExportVO`：35 个 String 字段，按 index 标注 `@ExcelProperty`，列顺序以饭火轮最新模板为准。
...
- 风险：多规格商品规格维度超过 10 个时数据截断，但模板本身只支持 10 维，属预期行为。
