...
- 导出列数、列名、列顺序与饭火轮模板 `dishes (9).xls` 完全一致（共 35 列）。
...
- 单规格商品（`specType == 0`）输出一行，第 5-7 列（外卖售价、店内价格、商品库存）及第 11-12 列（原价、商品销量）填充，第 13-35 列留空。
- 多规格商品（`specType == 1`）每个 SKU 输出一行，第 5-7 列留空，第 11-12 列填充，规格/SKU 列有值。
- 商品库存或 SKU 库存为不限制库存（`-1`）时，导出为 `100000`。
- 规格维度按 `store_product_attr.id` 升序排列，与 SKU 的 `sku` 字段按 `|` 拆分后的顺序对齐；最多支持 10 个维度，超出部分截断。
...
- 是否招牌固定输出 `2`。
- 查询按商品 `sort`、`id` 升序排列，导出排序列按结果顺序重新编号，避免源数据排序值全部为 0。
- 文件名为 `dishes.xls`。
...
- 导出文件可在 Excel/WPS 中正常打开，列头与模板 `dishes (9).xls` 完全一致。
- 单规格商品导出后第 5/6/7/11/12 列有值，第 13-35 列为空。
- 多规格商品导出后第 5/6/7 列为空，第 11/12 列及规格/SKU 列有值。
- 套餐商品不出现在导出结果中。
- 商家账号只能导出本店商品。

## Assumptions

- 饭火轮模板 35 列结构保持不变；第 11 列为“原价”，第 12 列为“商品销量”。
- 导出为一次性同步导出，不引入异步任务。
