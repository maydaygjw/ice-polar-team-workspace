...
- 规格维度按 `store_product_attr.id` 升序排列，与 SKU 的 `sku` 字段按逗号分隔后的顺序对齐；兼容历史竖线分隔数据，最多支持 3 个维度，超出部分截断。历史 SKU 缺少维度值时，按规格定义中的选项展开导出。
...
- 门店存在多规格商品但历史 SKU 缺少部分维度值时，使用该维度的已声明选项展开；没有可用 SKU 或规格定义时才按原规则不生成规格行。
