...
- `yshop-module-product-biz`：只保留商品、分类、SKU 和属性等核心能力；通过商品导入写入 API 为导入模块提供创建能力，不承载导入流程编排或删除能力。
...
7. 导入写入以批次为边界，成功、跳过、失败行均保存明细；删除只物理清理导入批次和明细，不触碰店铺、商品和分类。
8. 商品确认时以“非空外部商品 ID + 一级分类”作为商品身份；同一分类下的不同规格合并到同一个商品并创建多个 SKU，跨分类时创建商品副本。美团用户端模板忽略二级分类。
...
- 导入模块直接依赖 `store-api`、`product-api`、`infra-api` 和 Web/MyBatis/Excel starters，不直接依赖 `store-biz` 或商品表；商品模块通过 API 返回创建结果。
...
- 迁移只新增表和可空字段，回滚删除新增字段/表；导入记录删除只清理 import 表数据，已导入业务数据由店铺/商品管理页面负责。
