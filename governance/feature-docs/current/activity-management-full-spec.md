...
- 小程序码绑定 `periodId`，每次可重新生成，返回预计 48 小时有效的临时地址；不保存永久二维码业务记录。
...
外部小程序码请求只允许传 `periodId`，服务端固定页面路径和 scene，不接受 path、scene、appId、租户信息或外部身份参数。
...
- 二维码定位到 `periodId`，H5 ticket 只能一次兑换且不可跨租户。
