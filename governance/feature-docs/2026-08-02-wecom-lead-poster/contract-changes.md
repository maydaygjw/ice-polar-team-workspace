...
- 联系我创建和更新请求必须包含启用状态的商圈 ID；服务端读取该商圈的 `code` 并作为企业微信联系我配置的 `state` 参数。商圈代码为空时拒绝创建或更新。
- 联系我创建请求包含企业微信配置、商圈、单人/多人类型、成员 UserID 或部门 ID、备注和添加好友验证配置；本期仅支持二维码场景。
- 企业微信联系我场景值遵循官方接口定义：`scene=2` 表示通过二维码联系，`scene=1` 表示小程序联系；本期创建和更新固定使用 `scene=2`。
...
- `mp_wecom_contact_way`：保存联系我配置、所属商圈、二维码当前信息和本地二维码快照。历史同步记录的商圈允许为空；新增和编辑必须绑定商圈。
...
- `state` 固定为所选商圈代码，企业微信在客户联系回调中原样返回，用于解析商圈上下文。
