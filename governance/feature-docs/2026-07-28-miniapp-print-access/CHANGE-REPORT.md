...
- 配置：新增 `yshop.delivery.brick.*`（host/token/orderSourceType=YXG/callbackBaseUrl）；token 走环境变量。**merchantUserId/bizRegionCode/destinationId 不配置**，发布单时从店铺/订单关联目的地实时取（deliveryMerchantNo、商圈 code、目的地 code；店铺/地址上的本地 id 仅用于本地查询），需与 brick 主数据一致。
