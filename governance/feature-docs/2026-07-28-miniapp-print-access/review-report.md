...
| 项 | 结果 | 说明 |
|---|---|---|
| 契约一致性 | 通过 | 出向 publishById 三条铁律（sourceIdList 非空 / 顶层 sourceId 一致 / 8 位大写 UUID uid 查重）落到 `BrickDeliveryGateway.buildOrderVO`；入向回调按 `extra_params.deliveryUid` 校验 + `id+orderState` 幂等 + 秒回 200 |
