...
- 校验：`uid` 匹配本地配送单（`DeviceOrderDO.extra_params.deliveryUid`）；`merchantUserId` 属本地门店配送商户。
...
- 不改店铺表（merchantUserId/bizRegionCode 用现有门店/商圈；destinationId 用订单关联目的地）；不新增表/列/唯一约束；不动 ICE_MAKER 路径。
