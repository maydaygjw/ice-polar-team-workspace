...
选择店铺后，自动加载该店铺已配置的计费规则并回填表单；若该店铺从未配置，表单初始化为 4 个角色的默认空值（未启用、比例为 0、不承担手续费）。

**交互逻辑**：
- 进入页面时未选择店铺，规则表单区显示 `el-empty` 提示 "请先选择店铺"
- 选择店铺后，调用 `GET /admin-api/pay/profit-sharing-rule/list-by-shop?shopId=xxx`
- 若返回空列表（店铺从未配置），表单初始化为 4 行默认值：`status=0`（禁用），`percentage=0`，`feeBearer=0`
- 若返回已有规则，回填到对应角色的行（未返回的角色保持禁用、比例 0）
...
| 分账比例 (%) | `el-input-number` | `required`，`min=0`，`max=100`，精度 2 位小数；启用角色的比例之和必须 = 100 |
| 启用 | `el-switch` | 启用/禁用该角色；禁用后该角色不参与分账，且不得设为承担方 |
| 承担手续费 | `el-radio` | 仅启用的角色可选；启用角色中有且仅有一个为 `是`；默认值=否 |

手续费承担方交互：
- 4 行使用同一 `el-radio-group`，仅启用的角色可选；选中一个启用角色为承担方时，其余自动置为「否」。
- 禁用某角色时，若该角色当前为承担方，自动取消承担方状态并提示用户。
- 保存前前端校验：至少启用一个角色，启用角色比例和 = 100，且恰好 1 个启用角色为承担方。  
...
- **店铺无已配置规则**：表单初始化为默认空值（所有角色禁用、比例=0、不承担手续费），提示 "该店铺尚未配置分账计费规则，请填写并保存"
- **保存失败**：`message.error()` 显示后端返回的具体错误（如 "启用角色比例和不等于100" / "手续费承担方不唯一或未启用"）
- **比例和 ≠ 100**：前端实时显示差额提示（只统计启用角色），保存按钮在比例和 ≠ 100 时禁用
- **未启用任何角色**：保存按钮禁用，提示 "至少启用一个角色"
...
|--------|------|
| 角色完整性 | 至少启用一个角色 |
| 比例总和 | 启用角色 `sum(percentage)` 必须 = 100 |
| 手续费承担方 | 有且仅有一个启用角色 `feeBearer=1` |
