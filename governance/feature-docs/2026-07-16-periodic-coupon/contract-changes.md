...
管理后台允许编辑未开始和进行中的优惠券。进行中优惠券调整 `totalNumber` 时，`totalNumber` 表示新的发放总量，必须不小于已领取数量；保存后剩余库存按 `number = totalNumber - 已领取数量` 重算，因此支持增加或减少未领取库存。领取限制等模板字段的修改仅影响后续领取，已发放用户券继续使用领取时固化的有效期快照。已生成的一券一码兑换码会随库存增加补发、随未兑换库存减少标记删除；兑换码类型本身不可切换。
