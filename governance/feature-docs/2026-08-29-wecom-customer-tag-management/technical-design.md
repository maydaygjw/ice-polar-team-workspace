...
- `backend`：在现有 `WecomApiClient` 增加企业微信客户标签库 API 封装，并在联系人同步中解析跟进成员标签；新增管理端 Controller、Service、VO、联系人标签关系表和菜单权限迁移。
- `admin`：新增企业微信客户标签页面、API client 和标签组/标签表单。
- `miniapp`、`icepolar-dms`：N/A，本功能不经过小程序或设备系统。

## 关键决策

1. **标签库实时源**：标签库列表直接调用企业微信 `get_corp_tag_list`，不做本地标签库缓存；联系人标签关系随联系人同步保存快照，保证联系人列表可展示且不需要逐联系人实时请求。
...
4. **对象身份**：企业微信 `group_id`、`tag_id` 使用字符串；不转换为本地 Long。联系人标签关系按 `contact_id + follow_userid + tag_id` 保存。

## 必要流程

`accountId → 租户范围内配置 → CorpID/Secret → access_token → 企业微信标签接口 → 脱敏后的管理端 DTO`

客户联系人同步还会读取 `externalcontact/get` 返回的 `follow_user[].tags`，写入联系人标签关系快照；分页和详情接口分别返回去重联系人标签、跟进成员标签。
...
- 联系人标签名称是同步时的快照；企业微信标签名称变化后，需要重新同步客户联系人才能刷新名称。
