...
不建议为所有用户持续维护完整 LLM 画像。长尾用户的订单和行为数据较少，画像成本可能高于个性化收益。建议分为四层：

| 用户层级 | 推荐方式 |
|---|---|
| 新用户 | 冷启动、站点热门、当前时段和群体推荐 |
| 普通用户 | SQL 轻量统计和群体画像 |
| 活跃用户 | 轻量个性化画像 |
| 高价值/高潜用户 | LLM 离线完整画像和个性化重排 |

所有用户都可以保留低成本统计特征：最近订单数、最近下单时间、常购品类、常用价格区间、常用时段和最近购买商品。只有达到活跃或高潜条件的用户才生成和持续维护完整 LLM 画像。

用户画像可以动态晋级和降级：新增订单、画像过期或行为明显变化时更新；连续 60–90 天没有有效行为时降级为轻量统计或群体推荐。判断用户价值时不能只看消费金额，还要结合活跃度、复购倾向和潜在增长空间。

### 商品候选池规模建议

需要区分“全局商品语义库”“推荐全局候选池”和“发送给 LLM 的候选列表”：

#### 全局商品语义库

商品语义库用于生成商品标签、相似度计算和后续候选召回，是全局共享的基础库，不针对单个用户。

建议准入规则：

- 至少产生过 1 个有效支付订单的商品，进入基础语义标签库；
- 至少被 3 个不同用户购买，或至少产生 5 个有效订单的商品，进入稳定相似度计算库；
- 已下架、删除、没有有效商品信息，或只有取消/退款订单的商品，不进入主库；
- 从未被购买的新商品进入独立的“新品探索库”，不作为购买相似度证据。

初始规模不建议直接指定一个固定数量，应以覆盖最近 90–180 天有效订单商品的 80%–90% 为目标。商品量较小的站点可以全部纳入；商品量较大时，按站点、品类和销量分层保留。MVP 可先控制在每个站点约 500–2,000 个有效商品，再根据覆盖率、相似度质量和离线任务耗时调整。

#### 推荐候选集

商品语义库不是每次交给 LLM 的候选列表。候选列表应从商品语义库中按当前业务场景进一步召回：

| 阶段 | 建议规模 | 作用 |
|---|---:|---|
| 推荐全局候选池 | 每站点/时段 200–500 | 从商品语义库中生成的公共热门、新品、促销和可售商品候选 |
| 用户候选集 | 100–300 | 全局池叠加用户复购、偏好品类、价格和关联购买商品后去重 |
| 离线初排集 | 30–50 | 由 SQL/Java 确定性规则排序后作为 LLM 输入 |
| 离线 LLM 重排集 | 10–20 | 生成用户和场景对应的预排序推荐结果 |

MVP 可以先采用“全局池 300、用户候选 150、初排 40、重排 10”。实际规模应根据商品量、用户量、离线任务耗时和模型成本做实验调整。
