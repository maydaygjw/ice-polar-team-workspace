...
`device` 是通用设备管理平台，不以制冰机为边界。平台由 `device-core` 定义设备号、设备类型、租户归属、连接状态、占用/可用状态和通用设备订单；`device-ice` 负责制冰机能力。
...
- 设备类型、通信协议、供应商和业务能力必须分离，不能用制冰机字段或 DMS 路径定义通用设备模型。
- 设备连接状态与占用/可用状态必须分开；设备可以同时在线且占用。
- 通用设备订单只表达设备操作履约，不替代商城支付订单；涉及支付、库存、佣金和退款的业务仍归 `order`。
- 跨模块调用必须通过对应 `-api`，禁止订单模块依赖具体 `-biz` 或设备 Provider。
