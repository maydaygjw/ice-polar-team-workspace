...
本目录存放从各子项目收集的 **OpenAPI 静态快照**，作为 `governance/ARCHITECTURE.md` 中接口定义的事实来源。

> **原则**：接口路径、参数、响应结构以本目录中的 OpenAPI JSON 为准；业务语义（字段映射规则、错误码含义、架构原则）仍以 `governance/ARCHITECTURE.md` 中的「契约」章节为准。
...
2. **契约校验**：在 CI 中对比 `ARCHITECTURE.md` 引用的接口路径是否存在于 OpenAPI 中
3. **文档生成**：使用 Swagger UI / ReDoc 等工具渲染可视化文档

## 与 ARCHITECTURE.md 的关系

```
ARCHITECTURE.md（契约章节） ← 语义契约（手写，不可替代）
...
4. 如果变更涉及跨仓库语义（字段映射、错误码），同步更新 `ARCHITECTURE.md` 的「契约」章节
