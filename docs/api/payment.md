...
查询订单 → 确认 paid=0 → POST /app-api/order/pay
    ├─ 微信/支付宝/Adapay：使用 data 调起渠道支付
    └─ 余额：服务端同步扣款
支付操作结束 → GET /app-api/order/detail/{orderId}
    └─ paid=1 后才进入履约、打印或成功页
```

支付平台回调和前端支付结果不是同一件事。前端支付 SDK 返回成功后，仍必须重新查询订单详情；只有订单详情中的 `paid = 1` 才表示业务订单已确认支付。
...
服务端从订单读取实际应付金额和商品标题，前端不传金额。只有未支付订单（`paid = 0`）允许发起支付。
...
Adapay 小程序支付返回的是 Adapay 原始结果，微信小程序支付参数嵌套在 `expend.pay_info` 中，并且 `pay_info` 的类型是 JSON 字符串，不是对象。
...
`cash` 仅用于特定线下或后台业务，不属于普通 C 端支付流程。`integral` 等枚举值不应作为本接口的通用支付方式传入；客户端只使用本文列出的四种渠道。

## 支付渠道锁定

微信、支付宝和 Adapay 等第三方支付会创建外部支付单号。同一订单一旦存在未关闭的第三方支付尝试，服务端会锁定支付渠道，不能直接切换到另一渠道。
...
Adapay 重新发起支付时由服务端管理新的外部支付单号，前端始终使用同一个业务订单号 `uni`。
...
该地址由微信、支付宝或 Adapay 等支付平台调用，前端不要请求，也不要把它当作支付状态查询接口。支付成功后的业务订单状态由服务端回调和支付通知流程更新；前端只通过订单详情确认最终状态。

余额支付没有外部支付回调。

## 前端对接检查清单

- 支付前使用订单详情的 `paid` 和 `payPrice`，不在客户端改金额。
- `uni` 使用服务端返回的 `orderId`，打印订单也使用其 `orderNo` 作为订单号。
- 第三方支付返回成功、失败或取消后，都重新请求订单详情。
- 只有 `paid = 1` 才展示支付成功或开始后续履约。
